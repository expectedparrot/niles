...
- For a pipeline status pass, use `niles human-update --output update_job.ep`;
  select `--scope people|organizations|all` or filters when needed, then run the
  returned `publish_command` with EP.
